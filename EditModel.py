...
class EditModel(object):
  ...
  def insertEdits(self, word):
    ...
    ret = []
    for i in xrange(0, len(word)): #can insert at start of word
    	#The corrupted signal is this character
    	corruptLetters = word[i]
    	for ch in EditModel.ALPHABET:
    		#The correct signal is the corrupt signal followed by any other character in the alphabet
    		correctLetters = word[i] + ch
    		#Corrected word inserts ch after word[i]
    		correction = word[1:i+1] + ch + word[i+1:]
    		ret.append(Edit(correction, corruptLetters, correctLetters))
    return ret
  ...
  def transposeEdits(self, word):
    ...
    for i in xrange(0,len(word)-1):
    	...
    	correction = word[0:i] + word[i+1] + word[i] + word[i+2:]
    	ret.append(Edit(correction, corruptLetters, correctLetters))
    return ret
  ...
  def replaceEdits(self, word):
    ...
    for i in xrange(0, len(word)):
    	...
    	for ch in EditModel.ALPHABET:
    		...
    		correctLetters = ch
    		correction = word[0:i] + ch + word[i+1:]
    		ret.append(Edit(correction, corruptLetters, correctLetters))
    return ret
